...
export default function SendToMail({email, closeModal}) {
  return (
      <div className="w-full h-full bg-[#000]/20 flex justify-center absolute top-0 left-0 font-workSans">
          <div className="bg-[#F2F4F7] rounded-3xl sticky h-fit mt-[300px] pt[56px] pb-[162px] flex flex-col justify-center items-center text-center">
              <svg
                  onClick={closeModal}
              ...
              <img src={sendMail} alt="email sent gif" className="w-fit h-fit mb-10 mt-32"/>
              <p className="w-[70%] text-primary-900 text-2xl">Your video link has been sent to <span className="text-primary-500 font-medium">johnmark@gmail.com</span></p>
              <p className="w-[65%] text-[18px] mt-[100px]">
                  <span>Would you need to view this video later?</span>
                  <span>Save to your account now!</span>
              </p>
              <button type="button" className="text-primary-50 font-workSans px-8 py-4 rounded-lg bg-primary-500 text-base w-fit mx-auto mt-6">Save Video</button>
              <p className="font-semibold text-[#7E7E7E] mt-10">Don’t have an account? <span className="text-primary-500">Create account</span></p>
          </div>
    </div>
  )
}
